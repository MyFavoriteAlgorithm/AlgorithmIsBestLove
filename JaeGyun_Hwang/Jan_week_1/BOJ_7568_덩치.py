...
# 몸무게 리스트, 키 리스트 (각각의 사람은 동일한 인덱스 번호로서 구별한다)
weight_list = []
height_list = []
...
# 등수 저장 리스트 (키와 몸무게와 동일한 인덱스로 사람을 구분한다)
count_list = []

# 한명마다 전체 사람과 비교해서 키,몸무게가 더 클시 count +1
for i in range(N):
    count = 1
    for j in range(N):
        if weight_list[i] < weight_list[j]:#몸무게가 더 클 때
            if height_list[i] < height_list[j]:#키가 더 크면 카운트
                count+=1
    count_list.append(count) # 등수를 리스트에 저장
