...
두번째 줄부터 N개의 줄에 걸쳐 
각 줄에 사이트 주소와 비밀번호가 공백으로 구분되어 주어진다. 
...
N+2번째 줄부터 M개의 줄에 걸쳐 
비밀번호를 찾으려는 사이트 주소가 한줄에 하나씩 입력된다. 
...
pw_map = {}
for i in range(N):
    address, pw = map(str, sys.stdin.readline().rstrip().split())
    pw_map[address] = pw

for i in range(M):
    address = sys.stdin.readline().strip()
    #print('pw:',pw_map[address])
    print(pw_map[address])